# TypeScript project compositions
...
python3 ../harness/scripts/new_project.py compose example-project \
  --catalog scaffolds/components.json --preset react-vite --into /tmp/example-project
```

Use `--preset minimal` for language tooling without an application framework.
Add optional components with repeated `--component <name>` arguments: `rest`, `graphql`, `anthropic-server`, and `openai-server`.
The composer resolves component dependencies and rejects conflicts.
Server provider components cannot be combined with React/Vite.
...
python3 ../harness/scripts/validate_compositions.py scaffolds/components.json \
  --reports /tmp/frontend-harness-preset-validation
```
